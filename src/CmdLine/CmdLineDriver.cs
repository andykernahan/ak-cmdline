...
using Switch = AK.CmdLine.Impl.Switch;
...
namespace AK.CmdLine
{
    ...
    public class CmdLineDriver
    {
        ...
        private bool TryParseArguments()
        {
            ...
            while (State.HasArgs)
            {
                if (!TryCollectNextParameter(out var descriptor, out var value))
                {
                    return false;
                }
                if (!TryConvertParameterValue(value, descriptor, out var convertedValue))
                {
                    return false;
                }
        ...
        private bool TryCollectNextParameter(out ParameterDescriptor descriptor, out string value)
        {
            ...
            var arg = State.Args.Dequeue();
            if (Switch.TryParse(arg, out var @switch))
            {
        ...
        private IUsageWriter UsageWriter { get; }
        ...
        private sealed class ProcessState
        {
            ...
            {
                Args = new Queue<string>(args ?? Enumerable.Empty<string>());
            }
